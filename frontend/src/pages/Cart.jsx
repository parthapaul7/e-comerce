...
const Cart = () => {
  ...
  const getCart = async () => {
    const data = await axios.get("http://localhost:5000/api/carts/user", {
      headers: {
    ...
    setTotalAmount(calcultePrice(data.data.products));
  };
  ...
  //   totalAmount = calcultePrice(cart);
  // }
  useEffect(() => {
    getCart();
    // getTotal();
    ...
    <Container>
      ...
      <Wrapper>
        ...
        <Bottom>
          <Info>
            {cart.map((item,id) => (
              <Prod key={id} {...item} setUpDate={setUpDate} update={update} />
            ))}
          </Info>
          <Summary>
    ...
    <>
      <Product>
        ...
        <PriceDetail>
          ...
          <ProductPrice>{props.productId.price}</ProductPrice>
        </PriceDetail>
        <Delete onClick={e=>{deleteCart(e,props.productId._id)}}>
          <DeleteOutline />
        </Delete>
      </Product>
      <Hr />
